...
function usage() {
  ...
  echo "\t-d\tDebug mode. Disables simulated typing"
  echo "\t-n\tNo wait"
  echo "\t-w\tWaits max the given amount of seconds before proceeding with demo (e.g. `-w5`)"
  echo ""
}
...
function p() {
  ...
  if [[ -z $TYPE_SPEED ]]; then
    ...
    echo "\033[0m$cmd" | pv -qL $[$TYPE_SPEED+(-2 + RANDOM%5)];
  fi
...
function check_pv() {
  command -v pv >/dev/null 2>&1 || {
    ...
    echo "${RED}##############################################################"
    echo ""
    echo "${COLOR_RESET}Installing pv:"
    echo ""
    echo "${BLUE}Mac:${COLOR_RESET} $ brew install pv"
    echo ""
    echo "${BLUE}Other:${COLOR_RESET} http://www.ivarch.com/programs/pv.shtml"
    echo "${COLOR_RESET}"
    exit 1;
  }
}
